...
deploy_revision app['id'] do
  ...
  ssh_wrapper "#{app['deploy_to']}/deploy-ssh-wrapper" if ::File.exists?("#{app['deploy_to']}/id_deploy")

  restart do
    if ::File.exists?("#{app['deploy_to']}/current/tmp/pids/unicorn.pid")
      "god start unicorn_#{app['id']}"
    else
      "god restart unicorn_#{app['id']}"
    end
  end
